...
func NewKeyStore(config configs.JWTConfig) (*KeyStore, error) {
	signingKeyPath := config.GetPrivateKeyPath()
	verificationKeyPath := config.GetPublicKeyPath()
...
func (s *KeyStore) SigningKey() *rsa.PrivateKey {
	return s.signingKey
}

func (s *KeyStore) VerificationKey() *rsa.PublicKey {
